...
void StelMovementMgr::smoothPan(double deltaX, double deltaY, double time, bool s)
{
    flagCustomPan = s;
        if (s)
        {
            ...
            //double
            rateX = deltaX/time; //in degrees
            //double
            rateY = deltaY/time; // in degrees
            //qWarning() << "X speed is " << rateX;
            //qWarning() << "Y speed is " << rateY;
            //deltaAlt = rateY;
            //deltaAz = rateX;
            setFlagTracking(false);
            setFlagLockEquPos(false);
...
void StelMovementMgr::updateMotion(double deltaTime)
{
    ...
    if (flagCustomPan)
    {
        double conv = (3.14159265/180.0);
        deltaAz = rateX*conv*deltaTime;
        deltaAlt = rateY*conv*deltaTime;
        //qWarning() << deltaAz << ", " <<deltaAlt;
        //qWarning() << deltaTime;
        //qWarning() << depl;
        //counter++ ;
        //qWarning()<< counter;
