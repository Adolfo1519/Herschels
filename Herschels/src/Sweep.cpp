...
Sweep::Sweep()
    /*: m_name("My Sweep"),
      m_startRA("07h24m0.00s"),
      m_endRA("08h10m0.00s"),
      m_startDec("24d12m00.0s"),
      m_endDec("21d52m00.0s"),
      m_date("1787-01-24T22:00:30.0")*/
          /*m_startRA(""),
      m_endRA(""),
      m_startDec(""),
      m_endDec(""),
...
#endif
/* ********************************************************************* */
QString Sweep::name(void) const
{
    return m_name;
}

void Sweep::setName(const QString  aName)
{
	m_name = aName;
}
...
Sweep * Sweep::sweepFromSettings(const QSettings *theSettings, const int sweepIndex)
{
    ...
    sweep->setName(theSettings->value(prefix + "name", "").toString());

    QString output = "sweep from settings name is" + sweep->name();
    //qWarning() << output ;
...
void Sweep::writeToSettings(QSettings * settings, const int index)
{
    ...
    QString output = "sweep name is" + this->name();
    //qWarning() << output ;//theSettings->value(prefix+ "name", "").toString();
...
{
    //qWarning() << "Model Requested";
	Sweep* model = new Sweep();
    model->setName("Adolfo's Sweep");
